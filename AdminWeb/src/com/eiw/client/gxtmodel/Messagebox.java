...
public class Messagebox {
	 ...
	 * to the basic JavaScript alert prompt).
	 * 
	 * @param title
	 *            the title bar text
	 * @param msg
	...
	/**
	 * Displays a message box with a progress bar. This message box has no
	 * buttons and is not closeable by the user. You are responsible for
	 * updating the progress bar as needed via {@link Messagebox#updateProgress}
	 * 
	 * @param title
	 *            the title bar text
	 * @param msg
	 ...
	 */
	public static Messagebox progress(String title, String msg,
			String progressText) {
		Messagebox box = new Messagebox();
		box.setTitle(title);
		box.setMessage(msg);
	 ...
	 */
	public static Messagebox prompt(String title, String msg,
			boolean multiline, Listener<MessageboxEvent> callback) {
		Messagebox box = new Messagebox();
		box.setTitle(title);
		box.setMessage(msg);
		...
		box.setType(multiline ? MessageBoxType.MULTIPROMPT
				: MessageBoxType.PROMPT);
	 ...
	 * This can be used to block user interaction while waiting for a
	 * long-running process to complete that does not have defined intervals.
	 * You are responsible for closing the message box when the process is
	 * complete.
	 * 
	 * @param title
	 *            the title bar text
	...
	public void addCallback(Listener<MessageboxEvent> listener) {
		if (dialog == null) {
			if (listeners == null)
				listeners = new HashMap<EventType, Listener<MessageboxEvent>>();
			...
			dialog = new Dialog() {
				...
				protected void onRender(Element element, int index) {
					...
					if (type == MessageBoxType.PROMPT) {
						...
						textArea.getAriaSupport().setLabelledBy(
								getId() + "-content");
						...
						icon = null;
					} else if (type == MessageBoxType.PROGRESSS
							|| type == MessageBoxType.WAIT) {
						dialog.getAriaSupport().setDescribedBy("");
						progressBar = new ProgressBar();
						progressBar.getAriaSupport().setLabelledBy(
								getId() + "-content");
				...
				protected ComponentEvent previewEvent(EventType type,
					...
					if (ce instanceof WindowEvent) {
						...
						MessageboxEvent e = new MessageboxEvent(
								Messagebox.this, this, we.getButtonClicked());
	 ...
	 * 
	 * @param closable
	 *            false to hide the top-right close button
	 */
	...
	/**
	 * A string that will replace the existing message box body text (defaults
	 ...
	 * dialog. This is useful for setting a different minimum width than
	 * text-only dialogs may need (defaults to 250).
	 * 
	 * @param minProgressWidth
	 *            the min progress width
	 */
	...
	/**
	 * The text to display inside the progress bar if progress = true (defaults
	 * to "", pre-render).
	 * 
	 * @param progressText
	 *            the progress text
	 ...
	 * relevant on message boxes initiated via {@link #progress}.
	 * 
	 * @param value
