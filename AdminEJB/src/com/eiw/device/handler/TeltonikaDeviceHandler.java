...
public class TeltonikaDeviceHandler extends DeviceHandler {
	...
	private static final String STR_FMECO3 = "FMEco3", STR_PRO3 = "FMPro3",
			STR_TMT250 = "TMT250", STR_FMB920 = "FMB920", VERSION_ONE = "V1",
			VERSION_SEC = "V2";
	...
	public static Map<String, Long> preOdometerMap = new HashMap<String, Long>();

	public static Map<String, String> preLatLangOdometerMap = new HashMap<String, String>();

	Map<String, String> speedLogicMap = new HashMap<String, String>();
	...
	public void handleDevice() {
		...
		if (speedLogicMap.isEmpty()) {
			LOGGER.info("speedLogicMap First Time Enter");
			String[] speedLogic = fleetTrackingDeviceListenerBO
					.getPreferencesData("speedLogic", "demo").split(",");
			for (int i = 0; i < speedLogic.length; i++) {
				speedLogicMap.put(speedLogic[i], speedLogic[i]);
			}
		}
			...
			for (int i = 0; i < avlDataArray.length; i++) {
				...
				try {
					...
					LOGGER.error("PreparevehicleEvents: avlData String Substr"
							...
							+ latitudeAsString + " IMEI NO : " + imeiNo);
					return null;
				}
				 ...
				 * !deviceModel.equalsIgnoreCase(STR_TMT250)) { if (prevVE !=
				 * null) { vehicleEvent.setSpeed(prevVE.getSpeed()); } else {
				 * vehicleEvent.setSpeed(0); } } else {
				...
				// pointer
				/*
				 * double odometer = 0.0; if
				 * (!deviceModel.equalsIgnoreCase(STR_TMT250) &&
				 * vehicleEvent.getSpeed() != 0) { odometer =
				 * distanceMatrix(prevVE.getLatitude(), prevVE.getLongitude(),
				 * vehicleEvent.getLatitude(), vehicleEvent.getLongitude()); if
				 * (odometer >= 20000) { return null; }
				 * vehicleEvent.setOdometer((long) odometer); } else {
				 * vehicleEvent.setOdometer((long) odometer); }
				 */

				if (deviceModel.equalsIgnoreCase(STR_TMT250)
						&& vehicleEvent.getSpeed() != 0) {
					vehicleEvent.setOdometer((long) 0);
				}
				...
				try {
					for (int j = 0; j < ioDetailsfromVehicleEvent.length; j++) {
						...
						if (inputTypefromVehicleEvent.length > 1) {
							if (inputTypefromVehicleEvent[0]
								...
								sensorsValue.put("tripOdometer", Long
										.valueOf(inputTypefromVehicleEvent[1]));
							} else if (inputTypefromVehicleEvent[0]
									.equalsIgnoreCase("66")) {
								vehicleEvent
								...
								sensorsValue.put("BLEHumidity4", Long
									...
									.equalsIgnoreCase("24")) {
								sensorsValue.put("SpeedFromID", Integer
										.valueOf(inputTypefromVehicleEvent[1]));
							} else if (inputTypefromVehicleEvent[0]
									.equalsIgnoreCase("16")) {
								sensorsValue.put("TotalOdometer", Long
										.valueOf(inputTypefromVehicleEvent[1]));

								if (!deviceModel.equalsIgnoreCase(STR_TMT250)) {
									Long curODOMETER = Long
											.valueOf(inputTypefromVehicleEvent[1]);
									if (preOdometerMap.get(vehicleEvent.getId()
											.getVin()) != null) {
										Long currOdo = curODOMETER
												- preOdometerMap
														.get(vehicleEvent
																.getId()
																.getVin());
										// vehicleEvent.setOdometer(currOdo);
										sensorsValue.put("totalBasedOdometer",
												currOdo);
										preOdometerMap.put(vehicleEvent.getId()
												.getVin(), curODOMETER);
									} else {
										preOdometerMap.put(vehicleEvent.getId()
												.getVin(), curODOMETER);
								...
								if (vehicleEvent.getEngine() == true) {
									if ((int) avlData.getGpsElement()
											.getSpeed() == 0) {
										vehicleEvent.setSpeed(1);
									} else {
										vehicleEvent.setSpeed((int) avlData
												.getGpsElement().getSpeed());
									}
								} else {
					...
					if (!deviceModel.equalsIgnoreCase(STR_TMT250)) {
						if (vehicleEvent.getEngine()) {
							if (speedLogicMap.get(vehicle.getCompanyId()) == null) {
								vehicleEvent.setSpeed((int) avlData
										.getGpsElement().getSpeed());
								// LOGGER.error("SpeedWithOutLogic: "
								// + vehicle.getCompanyId() + "  "
								// + vehicle.getVin());
							} else {
								if (((int) avlData.getGpsElement()
										.getSatellites()) < 10) {
									// LOGGER.error("SpeedLogic: "
									// + vehicle.getCompanyId() + "  "
									// + vehicle.getVin());
									if (prevVE != null) {
										vehicleEvent
												.setSpeed(prevVE.getSpeed());
									} else {
										vehicleEvent.setSpeed(0);
									}
								} else {
					...
					if (!deviceModel.equalsIgnoreCase(STR_TMT250)) {
						Long latOdometer = 0l;
						if (preLatLangOdometerMap.get(vehicleEvent.getId()
								.getVin()) != null) {
							latOdometer = (long) distance(
									Double.valueOf(preLatLangOdometerMap.get(
							...
							sensorsValue.put("latLangOdometer", latOdometer);
							vehicleEvent.setOdometer(latOdometer);
							preLatLangOdometerMap.put(vehicleEvent.getId()
									.getVin(), vehicleEvent.getLatitude() + ","
									+ vehicleEvent.getLongitude());
						} else {
							if (vehicle.getVLatitude() != null) {
								latOdometer = (long) distance(
										vehicle.getVLatitude(),
										vehicle.getVLongitude(),
										vehicleEvent.getLatitude(),
										vehicleEvent.getLongitude());
								sensorsValue
										.put("latLangOdometer", latOdometer);
								vehicleEvent.setOdometer(latOdometer);
								preLatLangOdometerMap.put(vehicleEvent.getId()
										.getVin(), vehicleEvent.getLatitude()
										+ "," + vehicleEvent.getLongitude());
							} else {
								sensorsValue.put("latLangOdometer", 0l);
								vehicleEvent.setOdometer(0l);
								preLatLangOdometerMap.put(vehicleEvent.getId()
										.getVin(), vehicleEvent.getLatitude()
										+ "," + vehicleEvent.getLongitude());
				...
				prevVehicleeventMap.put(vehicleEvent.getId().getVin(),
						vehicleEvent);
				if ((deviceModel.equalsIgnoreCase(STR_FMECO3))
						|| (deviceModel.equalsIgnoreCase(STR_PRO3))) {
					if (isOdometer) {
	...
	public String sendCommand(String password, String model, String command,
		...
		try {
			...
			if (command.equalsIgnoreCase("cutOffEngine")) {
				...
				} else if (model.equalsIgnoreCase("FMB120")) {
					cutOffCommand = "setdigout 01";
				}
			...
			} else if (command.equalsIgnoreCase("rfidAddCall")) {
				commandStatus = null;
				String restoreCommand = "";
				if (model.equalsIgnoreCase("FMB120")) {
					restoreCommand = "setparam 30000:" + password;
				}
				String serverCommand = Codec12Format.encode(restoreCommand);
				// Equivalent Hex code for '$'
				byte[] buffer = Codec12Format
			...
			} else if (command.equalsIgnoreCase("rfidRemoveCall")) {
				commandStatus = null;
				String restoreCommand = "";
				if (model.equalsIgnoreCase("FMB120")) {
					restoreCommand = "setparam 30000:0";
				}
				String serverCommand = Codec12Format.encode(restoreCommand);
				// Equivalent Hex code for '$'
				byte[] buffer = Codec12Format
...
// <i class="fa fa-bell fa-3x icon-grey"><span class="badge">400</span></i>
//
//
//
//
