	...
	private List<Vehicleevent> prepareVehicleEvents(Vehicle vehicle,
		...
		try {
			...
			for (int i = 0; i != gpsElementCount; i++) {
				...
				//Old Data Skiping Code
				/*Date eventTime = TimeZoneUtil.getDateTimeZone(new Date(
						elements[i].getTimestamp()), region);
				Date currentTime = TimeZoneUtil.getDateTimeZone(new Date(),
						region);

				long diffDays = Math.abs(eventTime.getTime()
						- currentTime.getTime()) / 43200000;
				if (diffDays >= 1) {
					LOGGER.error("EventTimeStamp is Not In CurrentDay"
							+ eventTime);
					return null;
				}*/
				...
				for (String io : ioElements) {
					...
					if (ioProperty[0].equalsIgnoreCase("78")) {
						...
						if (previousOdometer != null
								...
								&& !previousOdometer.equalsIgnoreCase("")) {
