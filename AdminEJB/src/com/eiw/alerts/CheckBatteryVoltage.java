		...
		if (vehicleComposite.getDeviceModel().equalsIgnoreCase("AIS140")) {
			try {
				for (Vehicleevent vehicleEvent : vehicleEvents) {
					String deviceData = vehicleEvent.getTags();
					if (deviceData.startsWith("{")) {
						...
						if (obj.has("packetType")
							...
							Date date = vehicleEvent.getServerTimeStamp();
							String latlng = vehicleEvent.getLatitude() + ","
							...
							String description = "Alert%0DType : Low Battery%0Dvehicle:"
									+ plateNo
									+ "%0DBattery: "
									+ vehicleEvent.getBattery()
									+ " V %0DTime:"
									+ time;
							Vehiclealerts va = new Vehiclealerts();
							va.setAlerttype("Low Battery");
							va.setDescription(description);
							va.setEventTimeStamp(date);
							va.setLatlng(latlng);
							va.setSmsmobile(mobile);
			...
			if (initLow == 0) {
				try {
					Date date = ve0.getId().getEventTimeStamp();
					String latlng = ve0.getLatitude() + ","
							+ ve0.getLongitude();
					String time = TimeZoneUtil.getStrTZDateTime(date);
					String description = "Alert%0DType : Battery Disconnect%0Dvehicle:"
							+ plateNo
							+ "%0DBattery: 0 v%0DLimit : 10 v%0DTime:" + time;
					Vehiclealerts va = new Vehiclealerts();
					va.setAlerttype(AlertsManager.enumAlerts.OVERSPEED.name());
					va.setDescription(description);
					va.setEventTimeStamp(date);
