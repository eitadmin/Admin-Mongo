...
public class CheckEngineIdle implements CheckAlerts {
	/*
	 * Business Logic When Engine is ON and speed = 0 for more than given amount
	 * of time , then the vehicle is considered to be IDLE.
	 */
	...
	public CheckEngineIdle(AlertsManager alertsManager1) {
		alertsManager = alertsManager1;
		alertsEJBRemote = alertsManager.alertsEJBRemote;
	}

	public CheckEngineIdle(Alertconfig alertConfig, Date lastUpdatedTime) {
	...
	public void manageAlert(List<Vehicleevent> listOfEvent1,
		...
		idleInterval = Integer.valueOf(range);
		...
		AlertsEJB.LOGGER.debug("Inside CheckEngineIdle Alert ->");
		...
		for (Date date : dates) {
			try {
				...
				for (Vehicleevent velocal : vehicleevents) {
					...
					if ((speed == 0) && (engine)) {
						if (dateDiff > idleInterval) {
							String description = "Alert%0DType : Engine Idle Time%0Dvehicle:"
									+ plateNo
									+ "%0DLimit:"
									+ getSecInMin(idleInterval)
									+ "min %0DTime:" + eventTime;

							Vehiclealerts va = new Vehiclealerts();
							va.setAlerttype(AlertsManager.enumAlerts.IDLE
									.name());
