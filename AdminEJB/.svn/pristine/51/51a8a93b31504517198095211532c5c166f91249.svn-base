	...
	private Vehicleevent prepareVehicleEvents(Vehicle vehicle,
		...
		try {
			...
			if ((companytrackDevice.getCompanytrackdevicemodels().getId()
					...
					|| (companytrackDevice.getCompanytrackdevicemodels()
							.getId().getModelName().equalsIgnoreCase("TK003"))) {
				vehicleeventId.setEventTimeStamp(TimeZoneUtil.getDateTimeZone(
			...
			double latitude = (double) avlData.getConcoxGpsData().getLatitude();
			latitude = (double) Math.round(latitude * 10000) / 10000;
			double longitude = (double) avlData.getConcoxGpsData()
					.getLongitude();
			...
			if ((companytrackDevice.getCompanytrackdevicemodels().getId()
				...
				int ai1 = avlData.getConcoxGpsData().getAdtValue() * 10;
				if (ai1 < 1) {
					vehicleEvent.setAi1(ve.getAi1());
				} else {
					vehicleEvent.setAi1(ai1);
				}
		...
		try {
			...
			if (type.split("_")[0].equalsIgnoreCase("cutOffEngine")) {
				...
				if ((modelName.equalsIgnoreCase("KT-Mini"))
						...
						|| (modelName.equalsIgnoreCase("TK003"))) {
