...
public class AIS140DeviceHandler extends DeviceHandler {
	...
	public void handleDevice() {
		LOGGER.info("Entered Ais140 5444 five mins Handle Device:" + new Date());
		...
		try {
			...
			LOGGER.error("Ais140 5444 Data = " + data.getRawData());
			LOGGER.info("Test 1 Ais140 5444");

			while (true) {
				AIS140ByteWrapperV1 rawData = new AIS140ByteWrapperV1(
						clientSocketDis);
				rawData.unwrapDataFromStream();
				LOGGER.error("Ais140 5444 Data = " + rawData.getRawData());
	...
	private void insertService(AIS140ByteWrapper rawData,
		...
		try {
			...
			LOGGER.info("Ais140 5444 : Time:: " + position.getTime()
