...
public class TestDeviceHandler1 extends DeviceHandler {

	private static final Logger LOGGER = Logger.getLogger("listener");

	public void handleDevice() {
		LOGGER.info("handleDevice: Entered TestDeviceHandler_5429 Handle Device:"
				+ new Date());
		...
		try {
			...
			temp = new File(path + "DeviceTest_5429.txt");
			...
			LOGGER.error("TestDeviceHandler_5429: Close client socket" + e);
		} finally {
			try {
				writer.close();
			} catch (IOException e) {
		...
		LOGGER.info("TestDeviceHandler_5429: Ended successfully::: ");
	}
