...
public class SolarByteWrapper {
	...
	public SolarByteWrapper(DataInputStream in) {
		this.dis = in;
	}
	...
	public void unwrapDataFromStream() throws IOException {
		try {
			...
			LOGGER.info("Solar Data = " + this.rawData);
			...
			if (deviceData[0].equalsIgnoreCase("<LOCK")
					...
					|| deviceData[0].equalsIgnoreCase("<REQ")) {
				SolarDeviceHandler.commandStatus = deviceData[2];
				return;
			} else if (deviceData[0].equalsIgnoreCase("<Invalid Request")) {
				SolarDeviceHandler.commandStatus = "Invalid Request";
				return;
			}
			this.imei = this.deviceData[1];
			this.modbusData.read(this.deviceData);
		} catch (IOException e) {
			LOGGER.error("SolarByteWrapper : " + e);
			throw new IOException("SolarByteWrapper");
		}
	}
	...
	public static String crcCalculation(byte[] bytes) {
