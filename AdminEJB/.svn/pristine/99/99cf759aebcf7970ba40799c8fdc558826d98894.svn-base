package com.eiw.device.teltonika;
...
public class GpsElement {
	 ...
	 */
	public GpsElement(int x, int y, short altitude, short angle,
			byte satellites, short speed) {
		setX(x);
		setY(y);
	...
	/**
	 * @param angle
	 *            Direction in degrees, 0 equals north, increasing clockwise (45
	 *            equals north east)
	...
	public void setAngle(short angle) {
		if (angle < 0 || angle > 360) {
			// throw new IllegalArgumentException("Acceptable angle is [0..360 "
			// + angle);
		}
		this.angle = angle;
	}
	...
	public void setY(int y) {
		if (y < -90 * WGS_PRECISION || y > 90 * WGS_PRECISION) {
			throw new IllegalArgumentException("Acceptable y value is ["
					+ (-90 * WGS_PRECISION) + ".." + (90 * WGS_PRECISION)
					+ "] " + y);
		}
		this.y = y;
	}
	...
	public void setX(int x) {
		if (x < -180 * WGS_PRECISION || x > 180 * WGS_PRECISION) {
			throw new IllegalArgumentException("Acceptable x value is ["
					+ (-180 * WGS_PRECISION) + ".." + (180 * WGS_PRECISION)
					+ "] " + x);
		}
		this.x = x;
	}
	...
	public String toString() {
		return "[X=" + getX() + "] [Y=" + getY() + "] [Speed=" + getSpeed()
				+ "] [Angle=" + getAngle() + "] [Altitude=" + altitude
	 ...
	 *             from input stream
	 */
	public static GpsElement read(BitInputStream inputStream)
			throws IOException {
		...
		try {
			...
			throw new IOException("Unable to read GPS element. "
	 ...
	 */
	public static void write(BitOutputStream outputStream, GpsElement gpsElement)
			throws IOException {
